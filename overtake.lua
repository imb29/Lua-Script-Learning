...
local scale = 1.0  -- Scale factor for the UI window

function script.drawUI()
    local uiState = ac.getUiState()
    updateMessages(uiState.dt)

    local windowSize = vec2(400, 400) * scale
    ...
    local function speedMeter(ref)
        local meterWidth = 180 * scale
        ui.drawRectFilled(ref + vec2(0, -4), ref + vec2(meterWidth, 5), colorDark, 1)
        ...
        local speed = math.min(ac.getCarState(1).speedKmh, meterWidth)
        if speed > 1 then
            ui.drawLine(ref + vec2(0, 0), ref + vec2(speed, 0), colorAccent, 4)
        end
    end

    ui.beginTransparentWindow("overtakeScore", vec2(100, 100), windowSize)
    ui.beginOutline()
    ...
    ui.offsetCursorY(20 * scale)
    ui.pushFont(ui.Font.Main)
    local startPos = ui.getCursor()
    for i = 1, #messages do
        local m = messages[i]
        local f = math.saturate(4 - m.currentPos) * math.saturate(8 - m.age)
        ui.setCursor(startPos + vec2(20 * scale + math.saturate(1 - m.age * 10) ^ 2 * 50, (m.currentPos - 1) * 15 * scale))
        ui.textColored(
            m.text,
